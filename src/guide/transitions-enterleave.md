...
JavaScript のみを利用したトランジションの場合は、**`done` コールバックを `enter` と `leave` フックで呼ぶ必要があります**。呼ばない場合は、フックは同期的に呼ばれ、トランジションはただちに終了します。`:css="false"` を追加すると Vue は CSS 判定をスキップします。これにより、パフォーマンスが少し向上するだけでなく、誤って CSS ルールがトランジションに干渉するのを防ぎます。
...
あとで [コンポーネント間のトランジション](#transitioning-between-components) について説明しますが、`v-if`/`v-else` を使った通常の要素同士でもトランジションできます。最も共通の2つの要素のトランジションの例として、リストコンテナとリストが空と説明するメッセージの間で行うものがあります:
...
時には、オプションがありません、　　　そこで Vue は **transition modes** という非常に便利なユーティリティを提供しています。
...
これを利用して、以下のようにカードを折るような、より表現力豊かな動きをコーディネートすることができます。実際には相互に移動する2つの要素ですが、開始状態と終了状態は同じ拡大縮小であるため、水平方向に0となり、1つの流体運動のように見えます。このような微調整は、リアルな UI マイクロインタラクションに非常に役立ちます。
...
コンポーネント間のトランジションは、 key 属性が必要ではないのでさらに単純です。代わりに、ただ [動的コンポーネント](component-basics.html#dynamic-components): でラップするだけです:
